    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5 mb-5">
          <span className="mono text-[9px] font-medium tracking-[0.26em] text-accent">ECHO</span>
          <span className="flex-1 h-px" style={{ background: "oklch(0.575 0.200 47 / 0.38)" }} />
          <SimTicks sim={link.sim} size={9} />
          <span className="mono text-[9px]" style={{ color: "oklch(0.340 0.020 66)" }}>
            {link.sim.toFixed(2)}
          </span>
        </div>
        ...
        <div className="p-4" style={{ background: "var(--bg-raised)", border: "1px solid var(--scroll-thumb)" }}>
          <p className="mono text-[8px] tracking-[0.2em] uppercase mb-2.5" style={{ color: "oklch(0.562 0.016 74)" }}>
            THIS THREAD · {fmtDate(thisStartedAt)}
          </p>
          <p className="italic text-[16px] leading-[1.38]" style={{ color: "var(--ink-title)" }}>
            {thread.label}
          </p>
          {thread.summaryLine && thread.summaryLine !== thread.label && (
            <p className="text-[13px] leading-normal font-light mt-2" style={{ color: "oklch(0.482 0.018 71)" }}>
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="flex items-center gap-2 py-1.5">
            ...
            <span className="mono text-[8px] font-medium tracking-[0.2em] text-accent">
        ...
        <div className="p-4" style={{ background: "var(--bg-echo-b)", border: "1px solid oklch(0.575 0.200 47 / 0.38)" }}>
          <p className="mono text-[8px] tracking-[0.2em] uppercase mb-2.5 text-accent">
        ...
        {link.sharedEntities.length > 0 && (
          <div className="mt-4 pt-3.5" style={{ borderTop: "1px solid var(--hairline-strong)" }}>
            <p className="mono text-[8px] tracking-[0.2em] mb-2" style={{ color: "oklch(0.582 0.015 75)" }}>
              SHARED ENTITIES
            </p>
            <div className="flex flex-wrap gap-1.5">
              {link.sharedEntities.map((e) => (
                <span key={e} className="mono text-[8px] uppercase tracking-[0.13em] text-accent px-1.5 py-1" style={{ border: "1px solid oklch(0.575 0.200 47 / 0.42)" }}>
                  {e}
                </span>
              ))}
            </div>
        ...
        <div className="flex gap-2 mt-5">
          <button
            onClick={onOpen}
            className="flex-1 mono text-[9px] font-medium tracking-[0.18em] py-3 bg-accent"
          ...
          <button
            onClick={onClose}
            className="mono text-[9px] font-medium tracking-[0.18em] py-3 px-4"
